-- ============================================================================
-- OJO — PROYECTO CORRECTO: jpclzlzpkzmsmibweopn
--
-- Es el que usa el bot de Aura en producción (SUPABASE_URL en Railway). Si el
-- resultado no trae los buckets 'site-media', 'plantillas-media' o
-- 'comprobantes', estás en otro proyecto y NO hay que escribir nada contra él.
--
...
select 1 as orden, 'constraint:citas' as bloque,
...
union all
select 2, 'columna:citas', column_name || ' — ' || data_type
from information_schema.columns
where table_schema = 'public' and table_name = 'citas'

union all
select 5, 'columna:clientes', column_name || ' — ' || data_type
from information_schema.columns
where table_schema = 'public' and table_name = 'clientes'

union all
select 6, 'columna:bloqueos', column_name || ' — ' || data_type
from information_schema.columns
where table_schema = 'public' and table_name = 'bloqueos'

union all
select 7, 'columna:profiles', column_name || ' — ' || data_type
from information_schema.columns
...
union all
select 8, 'bucket', id || '  (public=' || public::text || ')'
